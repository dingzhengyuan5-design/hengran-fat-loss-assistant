...
const result=await evaluate(`(async()=>{
  ...
  const empty=document.querySelector('#planEmpty'),week1=document.querySelector('#trainingPanel').innerText;
  document.querySelector('#trainingPanel [data-week="1"]').click();const week2=document.querySelector('#trainingPanel').innerText;
  const mealBefore=document.querySelector('#mealsPanel .meal-card-v2').innerText;document.querySelector('#mealsPanel .replace-meal').click();const mealAfter=document.querySelector('#mealsPanel .meal-card-v2').innerText;
  const poster=document.querySelector('.video-poster');poster?.click();
  return {unsavedPrompt,saveState:document.querySelector('#saveState').textContent,planVisible:!document.querySelector('#planContent').hidden,emptyHidden:empty.hidden&&getComputedStyle(empty).display==='none',version:document.querySelector('#overviewVersion').textContent,headline:document.querySelector('#planHeadline').textContent,planHero:Boolean(document.querySelector('.plan-hero')),logoLoaded:document.querySelector('.brand-mark img').naturalWidth>0,summary:document.querySelector('#summaryPanel').innerText.slice(0,120),days:document.querySelectorAll('#mealsPanel [data-day]').length,weeks:document.querySelectorAll('#trainingPanel [data-week]').length,trainingProgressed:week1!==week2&&week2.includes('每组 +1次'),wholeMealChanged:mealBefore!==mealAfter,mealCards:document.querySelectorAll('.meal-card-v2').length,videoCards:document.querySelectorAll('.video-card-v2').length,videoFrames:document.querySelectorAll('.video-frame').length,profileSaved:Boolean(localStorage.getItem('hengran_profile_v2'))};
})()`);
...
await evaluate(`(()=>{const plans=JSON.parse(localStorage.getItem('hengran_plans_v2'));plans[0].schemaVersion=2;localStorage.setItem('hengran_plans_v2',JSON.stringify(plans))})()`);await call("Page.reload",{ignoreCache:true});for(let i=0;i<40;i++){try{if(await evaluate("JSON.parse(localStorage.getItem('hengran_plans_v2')).length===2"))break}catch{}await wait(150)}const migrated=await evaluate(`(()=>{const plans=JSON.parse(localStorage.getItem('hengran_plans_v2'));return {plans:plans.length,version:document.querySelector('#overviewVersion').textContent,schema:plans.at(-1).schemaVersion,reason:plans.at(-1).reason}})()`);
...
const output={mode:direct?"直接打开文件":"HTTP服务器",target:{url:page.url,title:page.title},initial,result,recovered,migrated,mobile,consoleErrors:errors};console.log(JSON.stringify(output,null,2));
...
if(!initial||initial.pages!==7||initial.foods!=="300"||!result.unsavedPrompt||result.saveState!=="已保存"||!result.planVisible||!result.emptyHidden||!result.planHero||!result.logoLoaded||result.days!==7||result.weeks!==4||!result.trainingProgressed||!result.wholeMealChanged||result.mealCards!==4||result.videoCards!==12||result.videoFrames<1||!recovered.visible||recovered.plans!==1||migrated.plans!==2||migrated.version!=="v2"||migrated.schema!==3||mobile.overflow||errors.length)process.exitCode=1;
